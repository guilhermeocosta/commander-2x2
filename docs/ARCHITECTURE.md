...
`robots.txt` is a static file in `public/`. The sitemap is generated by the `src/pages/sitemap.xml.ts` endpoint. It lists every `src/pages/*.astro` file automatically (through `import.meta.glob`). A `lastmod` map in that file gives pages backed by a data file that file's `updatedAt`.
